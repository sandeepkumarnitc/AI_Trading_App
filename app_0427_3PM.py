...
if bull_trap_reversal:
    set_signal("🔥 BUY PUT (BULL TRAP REVERSAL)", 100)

elif bear_trap_reversal:
    set_signal("🔥 BUY CALL (BEAR TRAP REVERSAL)", 100)

elif bull_trap and trap_score >= 60:
    set_signal("❌ BULL TRAP - AVOID CALL", 95)

elif bear_trap and trap_score >= 60:
...
# 📉 4. STRUCTURE FILTERS
elif abs(price - resistance) < 15 and df5["fut_volume"].iloc[-1] < df5["vol_ma"].iloc[-1]:
...
if st.session_state.last_signal == signal:
    ...
    # prevent rapid flip unless strong signal
...
elif "WAIT" in signal or "RANGE" in signal:
    decision = "WAIT"
...
if st.session_state.trade:
    ...
    # Prevent opposite signal during active trade
    if current_trade_type == "CE" and "PUT" in signal:
        signal = "⚠️ HOLD CALL - OPPOSITE SIGNAL IGNORED"

    if current_trade_type == "PE" and "CALL" in signal:
        signal = "⚠️ HOLD PUT - OPPOSITE SIGNAL IGNORED"
...
if (
    ...
    and lots > 0
    and signal_strength >= 70   # NEW FILTER
):
...
if decision == "ENTER TRADE":
    ...
    st.markdown(f"<h2 class='yellow center'>⚠️ {signal}</h2>", unsafe_allow_html=True)
